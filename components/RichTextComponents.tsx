...
export const RichTextComponents = {
    ...
    list: {
        bullet: ({ children }: any) => (
            <ul className="ml1- py-5 list-disc space-y-5">{children}</ul>
        ),
        number: ({ children }: any) => (
    ...
    block: {
        ...
        blockquote: ({ children }: any) => (
            <blockquote className="border-l-[#151725] border-l-4 pl-5 py-5 my-5">
                {children}
            </blockquote>
        ),
    },
        ...
        link: ({ children, value }: any) => {
            ...
            return (
                <Link
                    href={value.href}
                    rel={rel}
                    className="underline decoration-[#151725] hover:decoration-white"
                >
                    {children}
                </Link>
            );
